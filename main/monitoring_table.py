from tkinter import *
from Mon_Properties import FancyEntry
...
class SimpleTable(Frame):
    def __init__(self, parent, notebook, tools_notebook, tab_tools, tab5, response_table, tab_comparer, comp_table1, comp_table2, rows=1, columns=4):
        ...
        Label(self.table, text="Id", anchor='w',  relief='groove', font=self.tableFont, bg='#414550', fg='#9fb4bf').grid(row=0, column=1, sticky="nw", padx=2)
        Label(self.table, text="Status", anchor='w', relief='groove', font=self.tableFont, bg='#414550', fg='#9fb4bf').grid(row=0, column=2, sticky="nw", padx=2)
        Label(self.table, text="Content", anchor='w', relief='groove', font=self.tableFont, bg='#414550', fg='#9fb4bf').grid(row=0, column=3, sticky="nw", padx=2)
        Label(self.table, text="Length", anchor='w', relief='groove', font=self.tableFont, bg='#414550', fg='#9fb4bf').grid(row=0, column=4, sticky="nw")
        Label(self.table, text="", anchor='w', relief='groove', font=self.tableFont, bg='#414550', fg='#9fb4bf').grid(row=0, column=5, sticky="nw")


        self.table.grid_columnconfigure(1, weight=1)
        self.table.grid_columnconfigure(2, weight=1)
        self.table.grid_columnconfigure(3, weight=5)
        self.table.grid_columnconfigure(4, weight=1)
        self.table.grid_columnconfigure(5, weight=1)

    def add_row(self, status, url_vuln, length):
        self.row = self.row + 1
        row = self.row
        ...
        statusLabel.grid(row=row, column=2, sticky="nsew")
        contentLabel.grid(row=row, column=3, sticky="nsew")
        lengthLabel.grid(row=row, column=4, sticky="nsew")

        self.table.grid_columnconfigure(1, weight=1)
        self.table.grid_columnconfigure(2, weight=1)
        self.table.grid_columnconfigure(3, weight=3)
        self.table.grid_columnconfigure(4, weight=1)
